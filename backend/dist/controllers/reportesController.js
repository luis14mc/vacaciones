...
exports.exportarReporte = exports.getEstadisticasGenerales = exports.getReporteUsuarios = exports.getReportePorDepartamento = exports.getReporteSolicitudes = void 0;
const database_1 = __importDefault(require("../config/database"));
const getReporteSolicitudes = async (req, res) => {
    try {
...
exports.getReporteSolicitudes = getReporteSolicitudes;
const getReportePorDepartamento = async (req, res) => {
    try {
        const result = await database_1.default.query(`
      SELECT 
...
exports.getReportePorDepartamento = getReportePorDepartamento;
...
exports.getReporteUsuarios = getReporteUsuarios;
const getEstadisticasGenerales = async (req, res) => {
    try {
        const usuariosStats = await database_1.default.query(`
      SELECT 
      ...
      FROM usuarios
    `);
        const solicitudesStats = await database_1.default.query(`
      SELECT 
        COUNT(*) as total_solicitudes,
        COUNT(*) FILTER (WHERE estado = 'pendiente') as pendientes,
        COUNT(*) FILTER (WHERE estado = 'aprobada') as aprobadas,
      ...
      FROM solicitudes_vacaciones
    `);
        const departamentosStats = await database_1.default.query(`
      SELECT 
        COUNT(DISTINCT departamento) as total_departamentos
      FROM usuarios 
      WHERE departamento IS NOT NULL AND activo = true
    `);
        const solicitudesMesActual = await database_1.default.query(`
...
const exportarReporte = async (req, res) => {
    try {
        ...
        switch (tipo) {
            case 'solicitudes':
                const solicitudesResult = await database_1.default.query(`
          SELECT 
            s.id,
        ...
        `, fecha_inicio && fecha_fin ? [fecha_inicio, fecha_fin] : []);
                ...
                break;
            case 'usuarios':
                const usuariosResult = await database_1.default.query(`
          SELECT 
            u.nombre,
            u.apellido,
            u.email,
        ...
        if (formato === 'csv') {
            ...
            const rows = data.map((row) => Object.values(row).map(value => typeof value === 'string' && value.includes(',') ? `"${value}"` : value).join(','));
